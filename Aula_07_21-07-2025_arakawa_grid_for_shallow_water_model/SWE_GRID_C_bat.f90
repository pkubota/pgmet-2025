MODULE ModSWE
   ...
   REAL(KIND=8)   , PUBLIC, ALLOCATABLE :: bath(:,:) 
  ...
  PUBLIC :: Solve,Solve_Forward_Beta_plane,Solve_Forward_f_plane,&
	    ...
	    Solve_Forward_Beta_plane_Grid_C
...
CONTAINS
   ...
   SUBROUTINE DefineCI()
      ...
      ALLOCATE (bath(Idim,Jdim)) ; bath=0.0
      ...
      DO j=1,Jdim
         DO i=1,Idim
             ...
             h(i,j) =  MIN(-perturb*H0*exp (-((i - 0.5*Idim)/5.0)**2 - ((j - 0.5*Jdim)/5.0)**2),0.0)
             bath(i,j) = (0.75*H0*exp (-((i - 0.70*Idim)/5.0)**4 - ((j - 0.5*Jdim)/5.0)**2))
          !   bath(i,j) = (0.0*H0*exp (-((i - 0.70*Idim)/5.0)**4 - ((j - 0.5*Jdim)/5.0)**2))
   ...
   SUBROUTINE  Solve_Forward_Beta_plane_Grid_C(TermEqMomU, TermEqMomV, TermEqConH, u, v, h)

      REAL(KIND=8), dimension(1:Idim, 1:Jdim), intent( in) ::  u,  v,  h
      REAL(KIND=8), dimension(1:Idim, 1:Jdim), intent(out) :: TermEqMomU, TermEqMomV, TermEqConH
      
      INTEGER :: xb,xc,xf,i,j,yb,yc,yf
      REAL(KIND=8)    :: udux,vduy,vbar,fcov,gdhx,ru,vis2dudx,vis2dudy,	vbar2, vis2dhdx
      REAL(KIND=8)    :: udvx,vdvy,ubar,fcou,gdhy,rv,vis2dvdx,vis2dvdy,	ubar2, vis2dhdy
      REAL(KIND=8)    :: hdudx,hdvdy,dhudx ,dhvdy,gdbathx,gdbathy
      REAL(KIND=8)    :: betau,betav
      DO j=1,Jdim
         CALL index(j,Jdim,yb,yc,yf)
         DO i=1,Idim
            ...
            vduy  = vbar*((u(xc,yf) - u(xc,yb))/(2*DeltaY))
            !
            !
            ! d(u)
            ! -----   - f*v   = 0
	    ...
	    betav = -Beta*coordY(xc,yc)*vbar
            ...
            ! dt            H0-Bath    dx
            !
            gdbathx= g *((h(xc,yc)/(H0-((Bath(xc,yc)+Bath(xb,yc))/2.0))))*((Bath(xc,yc) - Bath(xb,yc))/(DeltaX))
            !                                                                          -                      -    
            !
            !                                                                          -                      -
            ! d(u)         d(u)         d(u)                 d(Eta)                   |d(d(u))       d(d(u))   |       h        d(Bath)
            ! -----  + u * ------ + v * -------  - beta*v  +g -------- + b*u - Neta * |-------- + ------------ | + g*----------*------- = 0
            ! dt           dx            dy                   dx                      |dxdx          dydy      |       H0-Bath    dx
            !                                                                          -                      -
            !TermEqMomU(i,j) = udux +  vduy + fcov + gdhx + ru + vis2dudx + vis2dudy + gdbathx

            TermEqMomU(i,j) = -1*(udux + vduy + betav + gdhx + ru + vis2dudx + vis2dudy + gdbathx)
            !
            !
            ! d(v)         d(v) 
            ! -----  + u * -------   =0
            ...
            !
            ubar=0.25*(u(xf,yc) + u(xc,yc) + u(xc,yb) + u(xf,yb) )
            udvx = ubar * ((v(xf,yc) - v(xb,yc))/(2*DeltaX))
            !
            !
            ! d(v)         d(v) 
            ! -----  + v * -------   =0
            ...
            ! dt            H0-Bath    dy
            !
            gdbathy= g *((h(xc,yc)/(H0-((Bath(xc,yc)+Bath(xc,yb))/2.0))))*((Bath(xc,yc) - Bath(xc,yb))/(DeltaY))

            !                                                                         -                      -
            ! d(v)         d(v)         d(v)                 d(Eta)                   |d(d(v))       d(d(v))   |      h        d(Bath) 
            ! -----  + u * ------ + v * -------  + beta*u  +g -------- + b*v - Neta * |-------- + ------------ |+ g*----------*------- =0
            ! dt           dx            dy                   dy                      |dxdx          dydy      |      H0-Bath    dy
            !                                                                          -                      -
            
            !TermEqMomV(i,j) = udvx + vdvy + fcou + gdhy + rv + vis2dvdx + vis2dvdy + gdbathy
            
            TermEqMomV(i,j) = -(udvx + vdvy + betau + gdhy + rv + vis2dvdx + vis2dvdy + gdbathy)
            !
            ! d(Eta)           d(u)    
            ! -----  + Eta * ---------  =0
            ! dt                dx     
	    ...
	    ubar= (u(xf,yc)+u(xc,yc))/2.0
         ...
         END DO
      END DO
   END SUBROUTINE Solve_Forward_Beta_plane_Grid_C
...
PROGRAM  Main
  ...
  OPEN(1,FILE='saidaC.bin',ACCESS='DIRECT',FORM='UNFORMATTED', RECL=lrec,&
...
CONTAINS
  ...
  SUBROUTINE Run(it)
      ...
      REAL(KIND=8) :: dt
        ...
        call Solve_Forward_Beta_plane_Grid_C(ku1, kv1, kh1, uo        , vo        , ho        )
        call Solve_Forward_Beta_plane_Grid_C(ku2, kv2, kh2, uo+ku1*dt2, vo+kv1*dt2, ho+kh1*dt2)
        call Solve_Forward_Beta_plane_Grid_C(ku3, kv3, kh3, uo+ku2*dt2, vo+kv2*dt2, ho+kh2*dt2)
        call Solve_Forward_Beta_plane_Grid_C(ku4, kv4, kh4, uo+ku3*dt , vo+kv3*dt , ho+kh3*dt )
