MODULE ModSWE
  ...
  PUBLIC :: Solve,Solve_Forward_Beta_plane,Solve_Forward_f_plane,&
            Solve_Forward_Beta_plane_Sphera,Solve_Forward_Beta_plane_Grid_A
...
CONTAINS
   ...
   SUBROUTINE  Solve_Forward_Beta_plane_Grid_A(TermEqMomU, TermEqMomV, TermEqConH, u, v, h)

      REAL(KIND=8), dimension(1:Idim, 1:Jdim), intent( in) ::  u,  v,  h
      REAL(KIND=8), dimension(1:Idim, 1:Jdim), intent(out) :: TermEqMomU, TermEqMomV, TermEqConH
      
      INTEGER :: xb,xc,xf,i,j,yb,yc,yf
      REAL(KIND=8)    :: udux,vduy,vbar,fcov,gdhx,ru,vis2dudx,vis2dudy, vis2dhdx
      REAL(KIND=8)    :: udvx,vdvy,ubar,fcou,gdhy,rv,vis2dvdx,vis2dvdy, vis2dhdy
      ...
      DO j=1,Jdim
	    ...
	    betav = -Beta*coordY(xc,yc)*v(xc,yc)
            ...
            !
            !
            gdhx  = g*((h(xf,yc) - h (xb,yc))/(2*DeltaX))
            !
            !
            !       
            !       
            ...
            ! dt                dx     
            !
            hdudx = H0 * ((u(xf,yc)  - u(xb,yc))/(2*DeltaX))
            !
            ! d(Eta)           d(v)
            ! -----  + Eta * -----------
            ! dt               dy
            !            
            hdvdy = H0 * ((v(xc,yf)  - v(xc,yb))/(2*DeltaY))
            !
            ! d(Eta)        d(Eta)
            ! -----  + u * ---------  =0
            ! dt             dx 
            !
            dhudx = u(xc,yc)*(h(xf,yc)  - h(xb,yc))/(2*DeltaX)
            !
            ! d(Eta)          d(Eta)
            ! -----   + v * ----------- =0
            ! dt              dy    
            !
            dhvdy = v(xc,yc)*(h(xc,yf) - h(xc,yb))/(2*DeltaY)
            !
            !                  -         -
            ! d(u)            |d(d(h))    |
            ! -----  - Neta * |--------   | = 0
...
PROGRAM  Main
  USE ModSWE, Only :    DefineCI,Solve,u,v,h,Idim,Jdim,DeltaT,&
   ...
   REAL(KIND=8),ALLOCATABLE :: un(:, :),uc(:, :), up(:, :)
   REAL(KIND=8),ALLOCATABLE :: vn(:, :),vc(:, :), vp(:, :)
   REAL(KIND=8),ALLOCATABLE :: hn(:, :),hc(:, :), hp(:, :)

  CALL Init()
  ALLOCATE(aux(1:Idim, 1:Jdim))
  ALLOCATE(un(1:Idim, 1:Jdim),uc(1:Idim, 1:Jdim),  up(1:Idim, 1:Jdim))
  ALLOCATE(vn(1:Idim, 1:Jdim),vc(1:Idim, 1:Jdim),  vp(1:Idim, 1:Jdim))
  ALLOCATE(hn(1:Idim, 1:Jdim),hc(1:Idim, 1:Jdim),  hp(1:Idim, 1:Jdim))
  un=u;uc=u;up=u 
  vn=v;vc=v;vp=v
  hn=h;hc=h;hp=h
  INQUIRE(IOLENGTH=lrec)aux
  OPEN(1,FILE='saida_GridA.bin',ACCESS='DIRECT',FORM='UNFORMATTED', RECL=lrec,&
...
CONTAINS
  ...
  END SUBROUTINE Init
   ...
   FUNCTION Filter_RAW(var_b,var_c,var_f)  RESULT (ok)
      IMPLICIT NONE
      REAL(KIND=8),  INTENT(INOUT) :: var_b (1:Idim, 1:Jdim)
      ...
      REAL(KIND=8), PARAMETER :: beta=0.5    !0.5 < beta <= 1
      INTEGER :: i,j
      INTEGER :: ok
      DO j=1,Jdim
         DO i=1,Idim
  ...
  SUBROUTINE Run(it)
      ...
      REAL(KIND=8) :: dt
        ...
        call Solve_Forward_Beta_plane_Grid_A(ku1, kv1, kh1, uo        , vo        , ho        )
        call Solve_Forward_Beta_plane_Grid_A(ku2, kv2, kh2, uo+ku1*dt2, vo+kv1*dt2, ho+kh1*dt2)
        call Solve_Forward_Beta_plane_Grid_A(ku3, kv3, kh3, uo+ku2*dt2, vo+kv2*dt2, ho+kh2*dt2)
        call Solve_Forward_Beta_plane_Grid_A(ku4, kv4, kh4, uo+ku3*dt , vo+kv3*dt , ho+kh3*dt )
   ...
   SUBROUTINE Run_LeapFrog(it)
      INTEGER, INTENT(IN  ) :: it
      REAL(KIND=8), dimension(1:Idim, 1:Jdim) :: lfu
      REAL(KIND=8), dimension(1:Idim, 1:Jdim) :: lfv
      REAL(KIND=8), dimension(1:Idim, 1:Jdim) :: lfh
      REAL(KIND=8) :: dt
      INTEGER      :: ires

        dt=DeltaT
       
        call Solve_Forward_Beta_plane_Grid_A(lfu, lfv, lfh, uc, vc, hc)
        up = un + 2*dt*lfu
        vp = vn + 2*dt*lfv
        hp = hn + 2*dt*lfh


        ! final step and time marching / new values for LeapFrog
	ires = Filter_RAW(un, uc, up)	
	ires=  Filter_RAW(vn, vc, vp)	
	ires=  Filter_RAW(hn, hc, hp)
